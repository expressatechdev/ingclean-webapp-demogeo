 ...
 * ACTUALIZADO: GPS Nativo para Capacitor (segundo plano)
 */
...
<body>
    <!-- Indicador GPS Nativo -->
    <div class="gps-native-badge" id="gpsNativeBadge">📱 GPS Nativo Activo</div>
    ...
    <script>
        ...
        // Mostrar badge si es GPS nativo
        if (isCapacitorApp) {
            console.log('📱 Ejecutando en Capacitor - GPS Nativo habilitado');
            document.getElementById('gpsNativeBadge').classList.add('show');
        }
        ...
        function initMap() {
            ...
            // Mostrar botón para activar GPS
        ...
        function showGPSActivationButton() {
            ...
            if (!btn) {
                ...
                btn.innerHTML = isCapacitorApp ? '📱 Toca para activar GPS Nativo' : '📍 Toca para activar GPS';
                btn.style.cssText = `
                    position: fixed;
                    top: 50%;
                    left: 50%;
        ...
        // =====================================================
        // FUNCIÓN PRINCIPAL GPS - DETECTA CAPACITOR O WEB
        // =====================================================
        function startMyLocationTracking() {
            ...
            if (isCapacitorApp && window.INGCleanGPS) {
                console.log('📱 Iniciando GPS NATIVO (Capacitor)');
                
                window.INGCleanGPS.init(
                    // Éxito
                    function(position) {
                        console.log('✅ GPS nativo:', position.coords.latitude, position.coords.longitude);
            ...
            // ========== GPS WEB (NAVEGADOR) ==========
                ...
                console.log('🌐 Iniciando GPS WEB (navegador)');
                ...
                navigator.geolocation.getCurrentPosition(
                    function(position) {
                        console.log('✅ GPS web:', position.coords.latitude, position.coords.longitude);
                        myLat = position.coords.latitude;
                        myLng = position.coords.longitude;
                        updateMyLocation(myLat, myLng);
                        sendLocationToServer(myLat, myLng);
                    ...
                    function(error) {
                        console.log('❌ Error GPS web:', error.code, error.message);
                        handleGPSError(error);
                    },
                    gpsOptions
                );
        ...
        function startContinuousTracking() {
            // Solo para GPS web - el GPS nativo ya tiene su propio watch
        ...
        function handleGPSError(error) {
            ...
            switch(errorCode) {
                    ...
                    if (isCapacitorApp) {
                        instructions = `
                            <div style="text-align:left; font-size:0.85rem; line-height:1.5;">
                                ...
                                1. Ve a <b>Configuración</b> del teléfono<br>
                                2. <b>Apps</b> → <b>INGClean</b><br>
                                3. <b>Permisos</b> → <b>Ubicación</b> → <b>Permitir siempre</b>
                            </div>
                        `;
                    } else {
                        instructions = `
                            <div style="text-align:left; font-size:0.85rem; line-height:1.5;">
                                <strong>Solución:</strong><br>
                                1. Ve a <b>Configuración</b> del navegador<br>
                ...
                case 2: // POSITION_UNAVAILABLE
                    ...
                    instructions = 'Verifica que el GPS esté activo en tu dispositivo';
                    break;
                case 3: // TIMEOUT
            ...
            showUnifiedGPSButton(errorCode === 1);
        }
        ...
        function showUnifiedGPSButton(isPermissionDenied) {
            ...
            if (isPermissionDenied) {
                btn.innerHTML = '🔄 Ya di permisos - Reintentar';
            } else {
                btn.innerHTML = isCapacitorApp ? '📱 Activar GPS Nativo' : '📍 Activar GPS';
            }
            ...
            btn.onclick = function() {
                ...
                setTimeout(() => {
                    if (!myLat || !myLng) {
                        btn.innerHTML = isPermissionDenied ? '🔄 Reintentar' : (isCapacitorApp ? '📱 Activar GPS Nativo' : '📍 Activar GPS');
                        btn.disabled = false;
        ...
        function showNavigationReminder() {
            ...
            toast.innerHTML = `
                ...
                ">
                    <strong>${isCapacitorApp ? '📱 GPS Nativo activo' : '💡 Recuerda'}</strong><br>
                    ${isCapacitorApp ? 'Tu ubicación se sigue enviando en segundo plano' : 'Vuelve a esta página para que el cliente vea tu ubicación'}
                </div>
            `;
        ...
        function confirmComplete() {
            ...
            // Detener GPS nativo al completar servicio
            if (isCapacitorApp && window.INGCleanGPS) {
                window.INGCleanGPS.stopForegroundService();
                console.log('📱 Servicio GPS en segundo plano detenido');
            }
        ...
        function confirmCancel() {
            ...
            // Detener GPS nativo al cancelar
            if (isCapacitorApp && window.INGCleanGPS) {
                window.INGCleanGPS.stopForegroundService();
            }
        ...
        // =====================================================
        // LIMPIEZA GPS AL SALIR - MODIFICADO PARA CAPACITOR
        // =====================================================
        window.addEventListener('beforeunload', function() {
            if (isCapacitorApp) {
                // En Capacitor NO detenemos el GPS - sigue en segundo plano
                console.log('📱 GPS nativo continúa en segundo plano');
            } else {
                // En web SÍ detenemos el watch
                    ...
                    if (data.success) {
                        ...
                        if (newStatus !== pollingOrderStatus) {
                            ...
                            if (newStatus === 'completed') {
                                // Detener GPS al completar
                                if (isCapacitorApp && window.INGCleanGPS) {
                                    window.INGCleanGPS.stopForegroundService();
                                }
                                window.location.href = 'index.php?completed=1';
                            } else if (newStatus === 'cancelled') {
                                // Detener GPS al cancelar
                                if (isCapacitorApp && window.INGCleanGPS) {
                                    window.INGCleanGPS.stopForegroundService();
                                }
                                window.location.href = 'index.php';
