...
<body>
    ...
    <script>
        ...
        let currentOrderCount = <?= count($pendingOrders) ?>;
        ...
        // Auto-refresh órdenes pendientes cada 5 segundos usando API
        <?php if (!$activeOrder): ?>
        function refreshPendingOrders() {
            fetch('../api/partner/status.php')
                ...
                .then(data => {
                    ...
                    // Si hay una orden activa ahora, recargar página
                    ...
                    if (data.pending_count > currentOrderCount) {
                        playNotificationSound();
                    }
                    currentOrderCount = data.pending_count;
        ...
        function updatePendingOrdersUI(orders) {
            const container = document.querySelector('.pending-orders') || document.querySelector('.empty-state')?.parentNode;
            if (!container) return;
            
            if (orders.length === 0) {
                // Mostrar estado vacío
                const existingOrders = document.querySelector('.pending-orders');
                if (existingOrders) {
                    existingOrders.outerHTML = `
                        <div class="empty-state">
                            <div class="empty-state-icon">📭</div>
                            <h3>No hay órdenes disponibles</h3>
                            <p>Mantente activo y disponible.<br>Te notificaremos cuando llegue una orden.</p>
            ...
            // Construir HTML de órdenes
            let html = '<div class="pending-orders">';
                    ...
                    <div class="order-card" id="order-${order.id}">
                        <div class="order-card-header">
                            <div>
                                ...
                                <p class="address">📍 ${order.client_address || 'Ubicación disponible al aceptar'}</p>
                            </div>
                            <div class="order-price-box">
                                <div class="order-earnings-main">💰 $${parseFloat(order.partner_earnings).toFixed(2)}</div>
                                <div class="order-earnings-label">Tu ganancia</div>
                            </div>
                        </div>
                        <button class="btn-accept" onclick="acceptOrder(${order.id}, this)">
        ...
        <?php if ($activeOrder): ?>
        // Polling para orden activa - detectar cuando cliente paga
        let activeOrderStatus = '<?= $activeOrder['status'] ?>';
        ...
        function checkActiveOrderStatus() {
            fetch('../api/partner/status.php')
                ...
                .then(data => {
                    ...
                    if (data.active_order) {
                        const newStatus = data.active_order.status;
                        
                        // Si el estado cambió
                        if (newStatus !== activeOrderStatus) {
                            playNotificationSound();
                            activeOrderStatus = newStatus;
                            
                            // Redirigir según el nuevo estado
                            if (newStatus === 'paid') {
                                window.location.href = 'active-service.php?order=<?= $activeOrder['id'] ?>';
                        ...
                        // Ya no hay orden activa
                        location.reload();
                    }
                })
                .catch(err => console.log('Error polling active order:', err));
        }
        
        setInterval(checkActiveOrderStatus, 5000);
        <?php endif; ?>
    </script>
