...
if ($userType === 'client') {
    $order = $db->fetchOne(
        "SELECT o.*, p.latitude, p.longitude, p.name as partner_name
         FROM orders o 
...
$responseData = [
    ...
    'latitude' => $latestLocation['latitude'] ?? $order['latitude'],
    'longitude' => $latestLocation['longitude'] ?? $order['longitude'],
